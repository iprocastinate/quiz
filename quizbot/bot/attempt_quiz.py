...
async def start(update, context):
    ...
    await update.message.reply_text(
        "<b>🔍 Find a Quiz</b>\n\n"
        "Which quiz would you like to take? Enter the quiz name.\n\n"
        "<i>💡 If it's not your quiz, add the creator's username after the name (e.g., 'GeneralQuiz john_doe').</i>",
        parse_mode='HTML'
    )
    return 'ENTER_QUIZ'
...
async def cancel(update, context):
    ...
    await update.message.reply_text(
        "<b>❌ Attempt Cancelled</b>\n\n"
        "You've left the quiz. Your progress was not saved. See you next time! 🙋‍♂️",
        parse_mode='HTML'
    )
    return ConversationHandler.END
...
async def enter_quiz(update, context):
    ...
    await update.message.reply_text(
        "<b>🎯 Let's Go!</b>\n\n"
        "Good luck with <b>'{}'</b>! 🙌\n"
        "<i>Type /cancelAttempt if you need to stop.</i>".format(quizname),
        parse_mode='HTML'
    )
...
async def enter_answer(update, context):
    ...
    if attempt.quiz.show_results_after_quiz:
        ...
        results_msg = ["<b>📊 Final Results</b>\n"]
        for i, (is_correct, question) in enumerate(attempt.user_points, 1):
            status = "✅ Correct" if is_correct else "❌ Wrong"
            results_msg.append(
                "<b>{}. {}</b>\n"
                "Question: {}\n"
                "Answer: {}\n".format(
                    i, status, question.question, question.correct_answer
                )
            )
        
        await update.message.reply_text("\n".join(results_msg), parse_mode='HTML')
...
async def ask_question(update, context):
    ...
    # print question
    progress = "<b>[Question {}/{}]</b>".format(attempt.current_index(), attempt.total_questions)
    await update.message.reply_text(
        "{}\n\n{}".format(progress, act_question.question),
